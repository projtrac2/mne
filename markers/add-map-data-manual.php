...
function disaggregation($outputid)
{
   ...
   $query_disaggregation =
      $db->prepare("SELECT o.id,o.mapping_type,d.output,i.indicator_disaggregation,i.indicator_name FROM tbl_project_details as o 
    INNER JOIN tbl_progdetails  as d ON d.id = o.outputid   
    INNER JOIN tbl_indicator  as i ON d.indicator = i.indid  
    WHERE  o.id=:outputid ORDER BY o.id ASC");
...
if (isset($_POST['submit_coords'])) {
   ...
   if (!in_array(false, $handler)) {
      $mapped = 1;
      $sql = "UPDATE tbl_projects SET mapped=:mapped , projstage=7 WHERE projid=:projid";
      $stmt = $db->prepare($sql);
