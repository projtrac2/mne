        ...
        if ($totalRows_rsProgram > 0 && $totalRows_rsStrategicPlan > 0) {
            ...
            if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addprogramfrm")) {
                ...
                if (validate_csrf_token($_POST['csrf_token'])) {
                    ...
                    if ($result) {
                        ...
                        $results = success_message('The Program was successfully created.', 2, $url);
                    } else {
                        $results = error_message("Error occured please try again later", 2, $url);
                    }
                } else {
                    $results = error_message("Error occured please try again later", 1, $url);
                }
            }
            ...
            function get_outputs_header()
            {
                ...
                for ($i = 0; $i < $strategic_plan_duration; $i++) {
                    ...
                    $thead .=
                        '<th colspan="2">' . $start_year . '/' . $end_year . '</th><input type="hidden" name="progyear[]" value="' . $start_year . '" />';
