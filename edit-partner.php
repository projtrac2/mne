...
if ($permission) {
    ...
    try {
        ...
        $msg = 'Partner successfully updated.';

        if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addpartnerfrm")) {
            $current_date = date("Y-m-d");
            if (!empty($_POST['partner']) && !empty($_POST['contactperson']) && !empty($_POST['title']) && !empty($_POST['designation']) && !empty($_POST['address']) && !empty($_POST['city']) && !empty($_POST['state']) && !empty($_POST['country']) && !empty($_POST['phone']) && !empty($_POST['email']) && !empty($_POST['comments']) && !empty($_POST['user_name'])) {
                $insertSQL = $db->prepare("UPDATE tbl_partners SET partner=:partner, contact=:contact, title=:title,designation=:designation, address=:address,city=:city, state=:state, country=:country, phone=:phone, email=:email, comments=:comments, updated_by=:user, date_created=:date WHERE id=:id");
                $insertSQL->execute(array(':partner' => $_POST['partner'], ':contact' => $_POST['contactperson'], ':title' => $_POST['title'], ':designation' => $_POST['designation'], ':address' => $_POST['address'], ':city' => $_POST['city'],  ':state' => $_POST['state'], ':country' => $_POST['country'], ':phone' => $_POST['phone'], ':email' => $_POST['email'], ':comments' => $_POST['comments'], ':user' => $_POST['user_name'], ':date' => $current_date, ':id' => $_POST['fndid']));
                            ...
                            <form id="add_partner" method="POST" name="addpartnerfrm" action="" enctype="multipart/form-data" autocomplete="off">
                                <fieldset class="scheduler-border">
                                    ...
                                    <div class="col-md-12">
                                        <label>Name *:</label>
                                        <div>
                                            <input name="partner" type="text" class="form-control" value="<?= $row_partner['partner'] ?>" placeholder="Enter name of financing institution" style="border:#CCC thin solid; border-radius: 5px" required />
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Contact Person *:</label>
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="contactperson" value="<?= $row_partner['contact'] ?>" placeholder="Enter contact person name" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Title *:</label>
                                        <div class="form-line">
                                            <select name="title" class="form-control show-tick" data-live-search="true" style="border:#CCC thin solid; border-radius:5px" required>
                                                ...
                                                while ($row_title = $query_title->fetch()) {
                                                    $selected = ($row_partner['title'] == $row_title['id'])  ? "selected" : '';
                                                ?>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="designation" value="<?= $row_partner['designation'] ?>" placeholder="Enter contact person designation" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <label>Address *:</label>
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="address" value="<?= $row_partner['address'] ?>" placeholder="Enter the financial institution address" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label>City *:</label>
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="city" value="<?= $row_partner['city'] ?>" placeholder="Enter the partner city" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label>State/Province *:</label>
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="state" value="<?= $row_partner['state'] ?>" placeholder="Enter the located state/province/county" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Country *:</label>
                                        <div class="form-line">
                                            <select name="country" class="form-control show-tick" data-live-search="true" style="border:#CCC thin solid; border-radius:5px" required>
                                                ...
                                                while ($row_country = $query_country->fetch()) {
                                                    $selected = ($row_partner['country'] == $row_country['id'])  ? "selected" : '';
                                                ?>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="phone" value="<?= $row_partner['phone'] ?>" placeholder="Enter partner phone number" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Email *:</label>
                                        <div class="form-line">
                                            <input type="email" class="form-control" name="email" value="<?= $row_partner['email'] ?>" placeholder="Enter partner email" style="border:#CCC thin solid; border-radius: 5px" required>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <label>Comments :</label>
                                        <div class="form-line">
                                            <textarea name="comments" cols="45" rows="5" class="txtboxes" id="comments" style="height:50px; width:98%; color:#000; font-size:12px; font-family:Verdana, Geneva, sans-serif"><?= $row_partner['comments'] ?></textarea>
                                            <script>
                                ...
                                <div class="row clearfix">
                                    ...
                                    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2" align="center">
                                        ...
                                        <input name="fndid" type="hidden" id="fndid" value="<?= $row_partner['id'] ?>" />
                                        <div class="btn-group">
