...
if (isset($_POST['ckids'])) {
	...
	$deleteQuery = $db->prepare("DELETE FROM `tbl_project_monitoring_checklist_score` WHERE formid=:formid AND monitoring_id=:monitoring_id");
	$results = $deleteQuery->execute(array(':formid' => $frmid, ":monitoring_id" => $monitoring_id));
	...
	for ($i = 0; $i < sizeof($_POST['ckids']); $i++) {
		...
		if ($total_checklistid == 0) {
			$statusquery = $db->prepare("INSERT INTO tbl_project_monitoring_checklist_score (mne_code,monitoring_id,taskid, checklistid, formid, score, level3, level4,responsible, date) VALUES (:mne_code,:monitoring_id,:taskid, :checklistid,  :formid, :score, :level3, :level4,:responsible,  :date)");
			$insertnot = $statusquery->execute(array('mne_code' => $mne_code, ":monitoring_id" => $monitoring_id, ':taskid' => $tskid, ':checklistid' => $ckids, ':formid' => $frmid, ':score' => $scores, ":level3" => $lev3id, ':level4' => $lev4id, ':responsible' => $responsible, ':date' => $scoredate));
		} else {
