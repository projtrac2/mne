...
function get_program_of_works_status($projid, $workflow_stage, $sub_stage)
{
    global $db, $user_designation, $user_name, $today, $due_date;
    //0 1 2 3 (Data entry)
    // 4 5 6 7 (Frequency)
    ...
    if ($user_designation == 1 || $user_designation <= 7) {
        if ($today > $due_date) {
            ...
            $activity_status = "<label class='label label-primary'>Pending Data Entry Activity Duration</label>";
            if ($sub_stage == 1) {
                $activity_status = "<label class='label label-success'>Assigned Activity Duration Data Entry</label>";
            } else if ($sub_stage == 2) {
                $activity_status = "<label class='label label-primary'>Pending Activity Duration Approval</label>";
            } else if ($sub_stage == 3) {
