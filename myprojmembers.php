...
try{
	...
	$query_rsMyP =  $db->prepare("SELECT *, FORMAT(projcost, 2), projstartdate AS sdate, projenddate AS edate, projcategory FROM tbl_projects WHERE user_name = '$user_name' AND projid = '$projectid_rsMyP'");
	$query_rsMyP->execute();		
	...
	$query_rsProgress =  $db->prepare("SELECT tbl_projects.projid, tbl_milestone.msid, COUNT(CASE WHEN tbl_task.status = 'Completed' THEN 1 END) AS `Completed`, COUNT(tbl_task.status) AS 'Total Status' FROM tbl_projects INNER JOIN tbl_milestone ON tbl_projects.projid=tbl_milestone.projid INNER JOIN tbl_task ON tbl_milestone.msid=tbl_task.msid WHERE tbl_projects.projid ='$colname_rsProgress' GROUP BY tbl_projects.projid");
	...
	$query_rsMSProgress =  $db->prepare("SELECT tbl_projects.projid, tbl_milestone.msid, COUNT(CASE WHEN tbl_milestone.status = 'Completed' THEN 1 END) AS `Completed`, COUNT(CASE WHEN tbl_milestone.status = 'In Progress' THEN 1 END) AS `In Progress`, COUNT(CASE WHEN tbl_milestone.status = 'Pending' THEN 1 END) AS `Pending`, COUNT(tbl_milestone.status) AS 'Total Status' FROM tbl_projects LEFT JOIN tbl_milestone ON tbl_projects.projid=tbl_milestone.projid WHERE tbl_projects.projid = '$colname_rsMSProgress' GROUP BY tbl_projects.projid");
	$query_rsMSProgress->execute();		
	$row_rsMSProgress = $query_rsMSProgress->fetch();
	$totalRows_rsMSProgress = $query_rsMSProgress->rowCount();

 
	$query_rsAdm =  $db->prepare("SELECT username, fullname, floc FROM `admin` WHERE username = '$colname_rsAdm'");
	$query_rsAdm->execute();		
	$row_rsAdm = $query_rsAdm->fetch();
	$totalRows_rsAdm = $query_rsAdm->rowCount();

	if (isset($_GET['projid'])) {
	  $colname_rsPMembers = $_GET['projid'];
	}

	$query_rsPMembers =  $db->prepare("SELECT * FROM tbl_projmembers WHERE projid = '$colname_rsPMembers' ORDER BY pmid ASC");
...
<body class="theme-blue" onload="getLocation()">
	...
	<!-- Inner Section -->
		<?php  include_once('myprojmembers-inner.php');?>
    <!-- #END# Inner Section -->
