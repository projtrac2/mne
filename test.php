    ...
    function get_subtask_status($projid)
    {
        ...
        if ($rows_rsWorkBreakdown > 0) {
            while ($row_rsWorkBreakdown = $query_rsWorkBreakdown->fetch()) {
                ...
                if ($complete == 0) {
                    if ($today < $subtask_start_date) { // substask start date haijafika 7/05 > 15/05 (Pending)
            ...
            while ($row = $query_workflow->fetch()) {
                ...
                if ($total_rsProjects > 0) {
                    while ($row_rsProjects = $query_rsProjects->fetch()) {
                        ...
                        } else if ($stage_id == 2 && $projid == 74) {
                            echo  "<br/> " . "     =>" . $project . "   <br/> " . $child_stage_id;

                            // ongoing 1) 18)
                            // - Monitoring
                            // - Substasks
                            // - Output complte, activity complete, issues (no issues) > 1
                            // - Inspection
                            //  c.end date 05/05/2024 >  new end date after extension 05/06/2024   Today /07/05/2024
