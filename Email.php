...
class Email
{
    ...
    function get_date($projid, $stage_id)
    {
    ...
    function get_members($projid, $workflow_stage, $substage_id)
    {
                ...
                if ($notification && $date) {
                    ...
                    if ($notification_type_id == 4) {
                        if ($substage_id == 0 || $substage_id == 2) {
                            ...
                            $user = get_director($directorate_id);
                            if ($user) {
                                $availability = $user->availability;
                                $user_id = $user->userid;
                                $fullname = $user->title . ' ' . $user->fullname;
                                $main_url = $availability == 1 ? $page_url : '';
                                $responsible = get_members($projid, $stage_id, $substage_id);
                                if ($responsible) {
                                    $responsible_id = $responsible->userid;
                                    if ($responsible->availability == 0) {
                                        $stand_in = get_stand_in($responsible_id);
                        ...
                        if ($substage_id == 0 || $substage_id == 2) {
                            ...
                            $user = get_members($projid, $stage_id, $substage_id);
                            if ($user) {
                                $availability = $user->availability;
                                $user_id = $user->userid;
                                $fullname = $user->title . ' ' . $user->fullname;
