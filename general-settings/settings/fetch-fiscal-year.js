...
$(document).ready(function () {
  ...
  manageItemTable = $("#manageItemTable").DataTable({
    ajax: "general-settings/selected-items/fetch-selected-fiscal-year-items.php",
    order: [],
    'columnDefs': [{
      'targets': [6],
      'orderable': false,
  ...
  $("#submitItemForm").on("submit", function (event) {
    ...
    if (year && fscyear && sdate && edate) {
      ...
      $.ajax({
        url: "general-settings/action/project-fiscal-year-action.php",
...
function editItem(itemId = null) {
  if (itemId) {
    ...
    $.ajax({
      url: "general-settings/selected-items/fetch-selected-fiscal-year-item.php",
      type: "post",
      data: { itemId: itemId },
      dataType: "json",
      success: function (response) {
