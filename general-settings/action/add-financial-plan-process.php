...
try {
    ...
    if (isset($_POST['getfinancier'])) {
        ...
        $query_rsFunding =  $db->prepare("SELECT *, f.financier AS finc, f.id AS fnid FROM tbl_myprojfunding m inner join tbl_financiers f ON f.id=m.financier WHERE projid = :projid");
        ...
        if ($totalRows_rsFunding > 0) {
            ...
            do {
                ...
                if ($remaining > 0) {
					echo '<option value="' . $funderid . '">' . $financier . '</option>';
                }
    ...
    if (isset($_POST['getfinancieredit'])) {
        ...
        for ($i = 0; $i < count($cfinid); $i++) {
            ...
            if ($totalRows_rsFunding > 0) {
                do {
                    $sourcat =  $row_rsFunding['sourcecategory'];
                    $funderid = $row_rsFunding['fnidFROM'];
                    $projamount = $row_rsFunding['amountfunding'];
                    $financier = $row_rsFunding['finc'];
                    ...
                    if ($ceiling > 0) {
						...
						$query_rsFunder = $db->prepare("SELECT * FROM tbl_funds WHERE financier='$financierId'");
						...
						if ($fndid == $funderid) {
							$option .= '<option value="' . $funderid . '" selected>' . $financier . '</option>';
						} else {
							$option .= '<option value="' . $funderid . '">' . $financier . '</option>';
						}
                    }
                } while ($row_rsFunding = $query_rsFunding->fetch());
            }
    ...
    if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add_budget_line_frm")) {
        ...
        $results  = $insertSQL->execute(array(":projstage" => $projstage, ":projid" => $projid));
        if ($results) {
            echo json_encode("Successfully Added Financial Plan");
        }
    }
