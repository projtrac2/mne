...
try {
	...
	if (isset($_POST["edititem"])) {
		...
		$updateQuery = $db->prepare("UPDATE tbl_budget_lines SET name=:name, grp=:grp, description=:description WHERE id=:itemid");
		$results = $updateQuery->execute(array(":name" => $name, ":grp" => $grp, ":description" => $description, ':itemid' => $itemid));
	...
	if (isset($_POST["deleteItem"])) {
		...
		$stmt = $db->prepare("SELECT * FROM `tbl_budget_lines` where id=:itemid");
		$stmt->execute([':itemid' => $itemid]);
		$stmt_results = $stmt->fetch();

		$stmt_results['status'] == 1 ? $updateStatus = '0' : $updateStatus = '1';

		$deleteQuery = $db->prepare("UPDATE `tbl_budget_lines` SET status=:status WHERE id=:itemid");
