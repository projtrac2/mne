...
try {
	...
	if (isset($_POST["edititem"])) {
		...
		$updateQuery = $db->prepare("UPDATE tbl_procurementmethod SET method=:method, description=:description WHERE id=:itemid");
	...
	if (isset($_POST["deleteItem"])) {
		...
		$stmt = $db->prepare("SELECT * FROM `tbl_procurementmethod` where id=:itemid");
		$stmt->execute([':itemid' => $itemid]);
		$stmt_results = $stmt->fetch();

		$stmt_results['status'] == 1 ? $updateStatus = '0' : $updateStatus = '1';

		$deleteQuery = $db->prepare("UPDATE `tbl_procurementmethod` SET status=:status WHERE id=:itemid");
