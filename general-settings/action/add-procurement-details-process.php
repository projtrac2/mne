	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add_budget_line_frm")) {
		...
		if (isset($_POST['contractrefno']) && !empty($_POST['contractrefno'])) {
			...
			$insertSQL->execute(array(
				":projid" => $projid, ":contractrefno" => $contractrefno, ":tenderno" => $_POST['tenderno'], ":tendertitle" => $tendertitle, ":tendertype" => $tendertype, ":tendercat" => $tendercat, ":tenderamount" => $tenderamount, ":procurementmethod" => $procurementmethod, ":evaluationdate" => $evaluation, ":awarddate" => $award,
