...
try{
	...
	if(isset($_POST["newitem"])){
		...
		$status = 1;
		$sql = $db->prepare("INSERT INTO tbl_tendercategory (category, description, status) VALUES(:category, :description, :status)");
	...
	if(isset($_POST["edititem"])){
		...
		$updateQuery = $db->prepare("UPDATE tbl_tendercategory SET category=:category, description=:description, status=:status WHERE id=:itemid");
	...
	if(isset($_POST["deleteItem"])){
		...
		$deleteQuery = $db->prepare("DELETE FROM `tbl_tendercategory` WHERE id=:itemid");
