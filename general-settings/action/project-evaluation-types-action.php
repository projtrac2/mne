...
try{
	...
	if(isset($_POST["newitem"])){
		...
		$status = 1;
		$sql = $db->prepare("INSERT INTO tbl_project_evaluation_types (type, description, active) VALUES(:type, :description, :status)");
	...
	if(isset($_POST["edititem"])){
		...
		$updateQuery = $db->prepare("UPDATE tbl_project_evaluation_types SET type=:type, description=:description, active=:status WHERE id=:itemid");
	...
	if(isset($_POST["deleteItem"])){
		...
		$deleteQuery = $db->prepare("DELETE FROM `tbl_project_evaluation_types` WHERE id=:itemid");
