...
try {
						...
						if (($ext != "exe") && ($_FILES["pfiles"]["type"][$cnt] != "application/x-msdownload")) {
							...
							if (!file_exists($filepath)) {
								if (move_uploaded_file($_FILES['pfiles']['tmp_name'][$cnt], $filepath)) {
									$fname = $newname;
									$mt = "uploads/main-project/" . $newname;
	...
	if (isset($_POST['get_source'])) {
		...
		if ($category == 2) {
			...
			$query_rsDonor = $db->prepare("SELECT * FROM tbl_donors d  INNER JOIN tbl_donor_grants g ON d.dnid = g.dnid WHERE g.financialyear=:financial_year");
			$query_rsDonor->execute(array(":financial_year" => $projfscyear));
			$row_rsDonor = $query_rsDonor->fetch();
			$totalRows_rsDonor = $query_rsDonor->rowCount();
			$donorid = [];
			do {
				$donor = $row_rsDonor['donorname'];
				$dndid = $row_rsDonor['dnid'];
				$rate = $row_rsDonor['exchangerate'];
				$funding_amount = $row_rsDonor['amount']; // check 

				// check amount that it has been used up 
				$query_rsUsedFunds = $db->prepare("SELECT sum(amountfunding) as amount FROM tbl_myprojfunding f INNER JOIN tbl_projects p ON  p.projid= f.projid WHERE projfscyear=:projfscyear AND financier=:id ");
				$query_rsUsedFunds->execute(array(":id" => $dndid, ":projfscyear" => $projfscyear));
				...
				if ($remaining > 0 && !in_array($dndid, $donorid)) {
					$inputs .= '<option value="' . $dndid . '">' . $donor . '</option>';
				}

				$donorid[] = $dndid;
			} while ($row_rsDonor = $query_rsDonor->fetch());
		} else if ($category == 1) {

			$query_rsFunder = $db->prepare("SELECT * FROM tbl_funder s  
			INNER JOIN tbl_funding f ON s.id =f.source WHERE f.financial_year=:financial_year AND s.type=:sourcecategory");
			...
			do {
				...
				$rate = $row_rsFunder['rate'];
				$funding_amount = $row_rsFunder['amtrec'] * $rate; // check 
	...
	if (isset($_POST['get_source_ceiling'])) {
		...
		$query_rsprojFunding =  $db->prepare("SELECT SUM(amountfunding) as amountfunding FROM tbl_myprojfunding  f INNER JOIN tbl_projects p ON  p.projid= f.projid WHERE p.projfscyear=:projfscyear AND  f.sourcecategory=:sourcecategory");
		$query_rsprojFunding->execute(array(":projfscyear"=>$projfscyear, ":sourcecategory" => $sourcecategory));
		...
		if ($category == 2) {
			...
			$donor = $row_rsDonor['donorname'];
			$dndid = $row_rsDonor['dnid']; 
			$rate = $row_rsDonor['exchangerate']; // check 
			$funding_amount = $row_rsDonor['amount'] * $rate; // check
			$remaining  = $funding_amount - $amountprojFunding;
		} else {
			$query_rsFunder = $db->prepare("SELECT * FROM tbl_funder s  INNER JOIN tbl_funding f ON s.id =f.source WHERE f.financial_year=:financial_year");
			$query_rsFunder->execute(array(":financial_year" => $projfscyear));
			$row_rsFunder = $query_rsFunder->fetch();
			$totalRows_rsFunder = $query_rsFunder->rowCount();
			$fndid = $row_rsFunder['id'];
			$funder = $row_rsFunder['name'];
