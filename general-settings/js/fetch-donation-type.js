...
$(document).ready(function() {
  ...
  manageItemTable = $("#manageItemTable").DataTable({
    ajax: "general-settings/selected-items/fetch-selected-donation-type-items",
    order: [], 
    'columnDefs': [{
      'targets': [4],
      'orderable': false,
  ...
  $("#submitItemForm").on("submit", function(event) {
    ...
    if (type == "") {
      $("#type").after(
        '<p class="text-danger">Donation Type Name field is required</p>'
      );
      $("#type")
        .closest(".form-input")
        .addClass("has-error");
    ...
    if (description == "") {
      $("#description").after(
        '<p class="text-danger">Donation Type Description field is required</p>'
      );
      $("#description")
        .closest(".form-input")
        .addClass("has-error");
    ...
    if (type && description) {
      ...
      $.ajax({
        url: "general-settings/action/project-donation-type-action",
...
function editItem(itemId = null) {
  if (itemId) {
    ...
    $.ajax({
      url: "general-settings/selected-items/fetch-selected-donation-type-item",
      type: "post",
      data: { itemId: itemId },
      dataType: "json",
      success: function(response) {
        ...
        $("#editItemForm")
          ...
          .bind("submit", function() {
            ...
            if (type == "") {
              $("#editType").after(
                '<p class="text-danger">Donation Type field is required</p>'
              );
              $("#editType")
                .closest(".form-input")
                .addClass("has-error");
