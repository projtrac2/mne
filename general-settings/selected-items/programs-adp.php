									...
									?>
											<div id="<?= $link ?>" class="tab-pane fade<?= $classstatusin ?>">
												<div style="color:#333; background-color:#EEE; width:100%; height:30px">
													<h4 style="width:100%"><i class="fa fa-list" style="font-size:25px;color:<?= $color ?>"></i> <?= $adp  . " (" . $year  . ")"; ?> Programs</h4>
												</div>
												<table class="table table-bordered table-striped table-hover js-basic-example dataTable">
													<thead>
														<tr class="<?= $linkclassbadge ?>" style="width:100%">
															...
															<th width="14%">Initial Budget (ksh)</th>
															<th width="18%">Adjusted Budget (ksh)</th>
													...
													<tbody>
														...
														if ($rows_count > 0) {
															...
															while ($row = $query_gadps->fetch()) {
																...
																$add_budget = $add_budget1;
																$edit_budget = $edit_budget1;
																$add_quarterly_targets = $add_quarterly_targets1;
																$edit_quarterly_targets = $edit_quarterly_targets1;
																...
																if ($norows_pbb > 0) {
																	...
																	if ($norows_pbbtargets > 0) {
																		...
																		$query_projects_count->execute();
																		$count_projects_count = $query_projects_count->rowCount();
																		if ($edit_budget && $count_projects_count == 0) {
																			$buttonunapprov .= '
																				<li>
																...
																$dept_array = array(10);
																$section_array = array(18);
																$directorate_array = array(41);
																$filter_open_department = $permissions->open_departmental_filter($dept_array, $section_array, $directorate_array);
																$filter_department = $permissions->filter_department_list($project_department, $project_section, $project_directorate);
																if ($filter_department || $filter_open_department) {
																	$sn++;
														?>
																	<tr>
			...
			<div class="modal-content">
				...
				<div class="modal-body" style="max-height:450px; overflow:auto;">
					<div class="div-result">
						<form class="form-horizontal" id="editpbbItemForm" action="general-settings/action/approve-PADP-action.php" method="POST">
							<br />
