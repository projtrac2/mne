										...
										?>
											<td width="50%" style="font-size:10px">
												<div class="btn-group" style="float:right">
													<a type="button" class="btn bg-brown waves-effect" href="myprojectmilestones?projid=<?php echo $row_rsMyP['projid']; ?>">Add Task</a>
												</div>
											</td>
										<?php
										}
													...
													<div class="table-responsive">
														<table class="table table-bordered table-striped table-hover">
															...
															<tbody>
																...
																do { 
																	$nm = $nm + 1;
																	$taskid = $row_rsMSTask['tbl_task.tkid'];
																	$query_rsStDate = $db->prepare("SELECT sdate FROM tbl_task where tkid='$taskid'");
																	...
																	if ($tkstartdate <= $current_date) {
																		/* mysql_select_db ($database_ProjMonEva,$ProjMonEva);
																		$sqlupdate="UPDATE tbl_task SET status='Task In Progress' WHERE tkid=$taskid";		
																		$retval = mysql_query($sqlupdate,$ProjMonEva); */
																		$sqlupdate = $db->prepare("UPDATE tbl_task SET status='Task In Progress' WHERE tkid='$taskid'");
																		$sqlupdate->execute();
																	}elseif ($tkstartdate > $current_date){
																		/* mysql_select_db ($database_ProjMonEva,$ProjMonEva);
																		$sqlupdate="UPDATE tbl_task SET status='Pending Task' WHERE tkid=$taskid"; 	
																		$retval = mysql_query($sqlupdate,$ProjMonEva);*/	
																		$sqlupdate = $db->prepare("UPDATE tbl_task SET status='Pending Task' WHERE tkid='$taskid'");
																		$sqlupdate->execute();
																	}
