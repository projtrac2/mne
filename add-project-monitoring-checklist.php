									...
									<tbody>
										...
										if ($totalRows_rsProjects > 0) {
											...
											do {
												...
												$query_rsMonitoring = $db->prepare("SELECT * FROM tbl_project_monitoring_checklist WHERE projid=:projid");
												$query_rsMonitoring->execute(array(":projid" => $projid));
												$row_rsMonitoring = $query_rsMonitoring->fetch();
												$totalRows_rsMonitoring = $query_rsMonitoring->rowCount();
												$assigned_responsible = check_if_assigned($projid, 999, $sub_stage, 1);
												...
												$activity = $totalRows_rsMonitoring == 0 ? "Add" : "Edit";
												if ($sub_stage == 0) {
													...
													<td>
														<div class="btn-group">
															...
															<ul class="dropdown-menu">
																...
																?>
																	<li>
																		<a type="button" href="add-project-design-checklist.php?projid=<?= $hashproc ?>" id="addFormModalBtn">
																			<i class="fa fa-plus-square-o"></i> <?= $activity ?> M&E Checklist
																		</a>
																	</li>
																<?php
																}
