...
if (isset($_GET["fnid"]) && !empty($_GET["fnid"])) {
    try {
        ...
        $fund_id = $fndid_array[1];

        $query_financierfunds =  $db->prepare("SELECT *, y.id AS yid, f.id AS fnid, fc.id AS FID, fc.type AS ftype FROM tbl_funds f inner join tbl_financiers fc on fc.id=f.funder inner join tbl_fiscal_year y on y.id=f.financial_year inner join tbl_currency c ON c.id=f.currency WHERE f.id=:fundid ORDER BY y.id DESC");
        $query_financierfunds->execute(array(":fundid" => $fund_id));
        $row_financierfunds = $query_financierfunds->fetch();
        $rows_financierfunds = $query_financierfunds->rowCount();

        if ($rows_financierfunds > 0) {
            $financier_id = $row_financierfunds['FID'];
            ...
            {
                global $fyear, $db;
                $query_rsYear =  $db->prepare("SELECT * FROM tbl_fiscal_year where id=:fyear ");
                $query_rsYear->execute(array(":fyear" => $fyear));
                $row_rsYear = $query_rsYear->fetch();
                return $row_rsYear ? $row_rsYear['year'] : '';
            }
            ...
            {
                global $logo, $db, $financial_year, $financier;
                ...
                if ($rows_company > 0) {
                    ...
                    $cover_page =
                        ...
                        <br/>
                        <hr/>
                        <h3 style="margin-top:10px;" > ' . $financier . '  ' . $financial_year . ' REPORT</h3>
                        <hr/>
                        <br/>
                        <br/>
                        <br/>
            ...
            function get_header()
            {
                global $logo, $financial_year, $financier;
                return
                    '<div style="text-align: right;">
                    <img src="' . $logo . '" height="80px" style="max-height: 100px; text-align: center;"/>
                    <p> <i><small>' . $financier . '   ' . $financial_year . ' REPORT</small></i></p>
                </div>';
            }
