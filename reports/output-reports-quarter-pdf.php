   ...
   <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
    ...
    if ($totalRows_indicators > 0) {
        while ($row_indicators = $query_indicators->fetch()) {
            ...
            $query_quarter_three_actual = $db->prepare("SELECT SUM(actualoutput) AS achieved FROM tbl_monitoringoutput WHERE opid=:indid AND (date_created>=:startq3 AND date_created<=:endq3)");
