...
try { 
    ...
    $mpdf->AddPage('L');
    $body = '
      <table class="table table-bordered table-striped table-hover" style="border:#EEE thin solid">
         <thead>
            <tr id="" style="padding-left:10px background-color:light-blue">
                  ...
                  <td width="7%"><strong id="colhead">&nbsp;&nbsp;Email</strong></td>
            ...
            if($totalRows_rsPrjTeam > 0){
               while ($row_rsPrjTeam = $query_rsPrjTeam->fetch()){ 
                     ...
                     <tr>
                        <td>'. $num .'</td>
                        <td>'.$fullname .'</td>
                        <td>'. $row_rsPrjTeam['designation'] .'</td>
                        <td>'. $ministry .'</td>
                        <td>'. $department .'</td>
                        <td>'. $row_rsPrjTeam['email'].'</td>
                        <td>'. $row_rsPrjTeam['phone'].'</td>
                     </tr>';
               }
            }else{
               $body .='
