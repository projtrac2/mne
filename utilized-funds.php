...
try{
	...
	$query_rsDnName = $db->prepare("SELECT * FROM tbl_donors WHERE dnid='$donorid'");
	$query_rsDnName->execute();
	$row_rsDnName = $query_rsDnName->fetch();

	$query_rsDonorGrant = $db->prepare("SELECT f.fund_code AS grantid, p.projname AS projname, p.projcategory AS cat, r.itemid AS itemid, d.amountpaid AS amntpaid, d.datepaid AS datepaid FROM tbl_funds f inner join tbl_projects p ON f.progid=p.progid INNER JOIN tbl_payments_request r ON p.projid=r.projid INNER JOIN tbl_payments_disbursed d ON d.fundsource=f.id WHERE f.sourcecategory='donor' AND f.sourceid='$donorid' ORDER BY f.id ASC");
    ...
    <section class="content" style="margin-top:-20px; padding-bottom:0px">
        <div class="container-fluid">
            <div class="block-header">
				<h4 class="contentheader"><i class="fa fa-slideshare" aria-hidden="true"></i> Donor Name: <?php echo $row_rsDnName["donorname"]; ?></h3></h4>
            </div>
            <!-- Draggable Handles -->
            <div class="row clearfix">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
						<?php
						include_once('utilized-funds-inner.php');
						?>
                    </div>
                </div>
            </div>
