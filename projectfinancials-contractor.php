...
try{		
	...
	$query_rsMilestone =  $db->prepare("SELECT p.*, m.msid, m.milestone, m.progress, m.status, m.sdate, m.edate, s.statusname FROM tbl_projects p INNER JOIN tbl_milestone m ON p.projid=m.projid INNER JOIN tbl_status s ON s.statusid=p.projstatus WHERE p.deleted='0' AND m.paymentstatus = 0 AND p.projcategory = '2' AND m.status = 5 AND m.inspectionscore = '100' GROUP BY m.milestone");
...
.dd3-handle:hover { background: #ddd; }
	...
	<script type="text/javascript">
	function CallMlstPaymentRequest(msid)
	{
		$.ajax({
			type: 'post',
...
<body class="theme-blue">
	...
	<div class="modal fade" id="commModal" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header" style="background-color:#03A9F4">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h3 class="modal-title" align="center"><font color="#FFF">Payment Request Comments</font></h3>
				</div>
	...
	<!-- Inner Section -->
		<?php  include_once('projectfinancials-contractor-inner.php');?>
    <!-- #END# Inner Section -->
