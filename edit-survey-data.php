...
require 'authentication.php';
	...
	try {
		...
		$query_rsUsers = $db->prepare("SELECT ptid FROM tbl_projteam2 t inner join tbl_users u on u.pt_id=t.ptid WHERE username = :username");
		$query_rsUsers->execute(array(":username" => $user_name));
		...
		if(isset($_POST['submit'])){
            ...
            // delete coordinates 
			$deleteQuery = $db->prepare("DELETE FROM `tbl_project_form_markers` WHERE form_id=:form_id AND respondent=:respondent");
            $results = $deleteQuery->execute(array(':form_id' => $form_id, ":respondent"=>$respondent)); 
 
            // delete form data 
			$deleteQuery = $db->prepare("DELETE FROM `tbl_indicator_baseline_values` WHERE form_id=:form_id AND respondent=:respondent");
			$results = $deleteQuery->execute(array(':form_id' => $form_id, ":respondent"=>$respondent));
			...
			for($i=0; $i< count($measurement_variable); $i++){
				...
				$result = 	$insertSQL->execute(array(":form_id"=>$form_id,":key_unique"=>$key, ":level3"=>$level3, ":location"=>$location,":measurement_variable"=>$measurement_variable_id,
					 ":disaggregations"=>$disaggregations,":value"=>$value, ":respondent"=>$respondent, ":form_type"=>$form_type, ":category"=>$category));
				}
			}
            
			if($result){
                $msg = 'The survey data was successfully updated.';
                $results = "
				<script type=\"text/javascript\">
					...
					setTimeout(function(){
							window.location.href = 'view-survey-tasks.php';
						}, 5000);
				</script>";
			}
		}
		...
		function printNode($node,$category,$respondent, $placeholder, $level=0, $islast=false, $element_arr=array()) { 
			GLOBAL $db, $indid, $disaggregated, $form_id;  
			$id =$node['id']; 
			$depth =$node['parent']; 
			$total = count($node);
			static $c =0;   
			...
			if(empty($last_id)){
				...
				if($id !="root" && $id !=1){   
					do{                
						...
						do{      
                            $measurement_variable = $row_measurement_variables['id'];
							$disa = $row['id'];  
                            ...
                            $query_rs_value = $db->prepare("SELECT * FROM tbl_indicator_baseline_values  
							WHERE  form_id='$form_id' 
							AND measurement_variable='$measurement_variable'
							AND disaggregations='$disa'  
							AND type='$category'  
							AND key_unique='$flex'  
							AND respondent='$respondent' ORDER BY id");

                            $query_rs_value->execute();
                            $indparen_value = $query_rs_value->rowCount();
                            $row_value = $query_rs_value->fetch();  
							$value = $row_value['value']; 
								...
								<div class="form-input">
										<input type="number" name="value'.$row_measurement_variables['id'].'[]" id="outputcost" value="'.$value.'" placeholder="'.$placeholder.'" class="form-control" >
									</div>
								</div>'); 
								foreach($node['children'] as $child) { 
									printNode($child,$category,$respondent, $placeholder,$level =0, $islast=false,$element_arr); 
								}
				...
				if($id !="root" &&  $id !=1){ 
					...
					do{              
						...
						foreach($node['children'] as $child) { 
							...
							printNode($child,$category,$respondent,$placeholder, $level++, $islast1,$element_arr);  
						}
					}while($row = $query_rschild->fetch()); 
				} else{
					foreach($node['children'] as $child) { 
		...
		function non_disaggregated($indid, $category,$respondent, $placeholder){
			GLOBAL $db, $form_id;
			$query_measurement_variables = $db->prepare("SELECT * FROM  tbl_indicator_measurement_variables WHERE indicatorid = '$indid' AND category='$category' ORDER BY id");
			$query_measurement_variables->execute();
			$row_measurement_variables = $query_measurement_variables->fetch();
			...
			if($totalrow_measurement_variables > 0){ 
				do{ 
                    ...
                    $query_rs_value = $db->prepare("SELECT * FROM `tbl_indicator_baseline_values` WHERE form_id =:form_id AND measurement_variable =:measurement_variable AND respondent=:respondent");
                    $query_rs_value->execute(array(":form_id"=>$form_id,":measurement_variable"=>$measurement_variable,":respondent"=>$respondent));
                    $indparen_value = $query_rs_value->rowCount();
                    $row_value = $query_rs_value->fetch();  
                    $value = $row_value['value'];  
		...
		$placeholder="";
		$stid=$id="";$indid="";$projid=""; $userid="";$form_id=""; $user="";$created_at=""; $location_name="";
		$form_body="";$projcode="";$form_name="";$projname="";$typeName="";$indicator_name="";$measurement_unit="";
		$data_source="";$respondent="";$phone="";
		$state=''; $locationName='';
        ...
        if(isset($_GET['formid'])){
			...
			if($form_type == 9){
				...
				if($form_type == 1){
					$placeholder ="Enter Assement Value"; 
				}else if($form_type == 2){
 			...
 			<div class="row clearfix">
 				...
 				<div class="col-lg-12 col-md-3 col-sm-12 col-xs-12">
 					<div class="card">
 						<!-- <div class="body"> -->
 						<?php
							include_once('edit-survey-data-inner.php');
						?>
 						<!--</div> -->
 					</div>
 				</div>
