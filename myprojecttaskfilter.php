...
require 'authentication.php';
	...
	if(isset($_GET["prg"])){
		...
		if(isset($_GET["edit"])){
			if($prog == 1){
				$msg = "Sorry, this task is being monitored hence can not be edited!!";				
				$result = "<script type=\"text/javascript\">
					...
					setTimeout(function(){
						window.location.href = 'myprojecttaskfilter?projid=$prjid&msid=$mslid';
					}, 6000);
				</script>";
			}elseif($prog == 0){
				$insertGoTo = "updatemytask?tkid=" . $tskid . "";
		...
		elseif(isset($_GET["del"])){
			if($prog == 1){
				$type="error";
				$msg = "Sorry, this task is being monitored hence can not be deleted!!";				
				$result = "<script type=\"text/javascript\">
					swal({
						title: \"Error!\",
						text: \" $msg \",
				...
				$insertGoTo = "deletetask?tskid=" . $tskid . "&mlid=" . $mslid . "&projid=" . $prjid . "";
				header(sprintf("Location: %s", $insertGoTo));
			}	
		}	
	}
	...
	if (isset($_GET['msid'])) {
		$pmsid_rsMSTask = $_GET['msid'];
	}
		
	$query_rsMyP =  $db->prepare("SELECT *, projcategory FROM tbl_projects WHERE deleted='0' AND user_name = '$user_name' AND projid='$projectid_rsMyP'");
	$query_rsMyP->execute();		
	$row_rsMyP = $query_rsMyP->fetch();
	$projfstmonitordate = $row_rsMyP["projfirstmonitor"]; 
	...
	$query_rsMSTask =  $db->prepare("SELECT tbl_projects.projid, tbl_milestone.msid, tbl_milestone.status AS mlstatus, tbl_milestone.msid, tbl_milestone.milestone, tbl_task.task, tbl_task.tkid, tbl_task.status, tbl_task.taskbudget, tbl_task.progress AS tskprogress, tbl_task.sdate AS stdate, tbl_task.edate AS endate FROM tbl_projects INNER JOIN tbl_milestone ON tbl_projects.projid=tbl_milestone.projid INNER JOIN tbl_task ON tbl_milestone.msid=tbl_task.msid WHERE tbl_projects.deleted='0' AND tbl_milestone.msid = '$pmsid_rsMSTask' ORDER BY tbl_task.sdate ASC");
	$query_rsMSTask->execute();		
	$row_rsMSTask = $query_rsMSTask->fetch();
		
	$query_rsProgress =  $db->prepare("SELECT tbl_projects.projid, tbl_milestone.msid, COUNT(CASE WHEN tbl_task.status = 'Completed' THEN 1 END) AS `Completed`,  COUNT(tbl_task.status) AS 'Total Status' FROM tbl_projects  INNER JOIN tbl_milestone ON tbl_projects.projid=tbl_milestone.projid INNER JOIN tbl_task ON tbl_milestone.msid=tbl_task.msid WHERE tbl_projects.deleted='0' AND tbl_projects.projid = '$projectid_rsMyP' GROUP BY tbl_projects.projid");
	$query_rsProgress->execute();		
	$row_rsProgress = $query_rsProgress->fetch();
	$totalRows_rsProgress = $query_rsProgress->rowCount();
		
	$query_rsMSProgress =  $db->prepare("SELECT tbl_projects.projid, tbl_milestone.msid, COUNT(CASE WHEN tbl_milestone.status = 'Completed' THEN 1 END) AS `Completed`, COUNT(CASE WHEN tbl_milestone.status = 'In Progress' THEN 1 END) AS `In Progress`, COUNT(CASE WHEN tbl_milestone.status = 'Pending' THEN 1 END) AS `Pending`, COUNT(tbl_milestone.status) AS 'Total Status' FROM tbl_projects  INNER JOIN tbl_milestone ON tbl_projects.projid = tbl_milestone.projid  WHERE tbl_projects.deleted='0' AND tbl_projects.projid = '$projectid_rsMyP' GROUP BY tbl_projects.projid");
...
<head>
    ...
    <title>Projtrac M&E - Milestone Tasks</title>
    <!-- Favicon-->
...
<body class="theme-blue" onload="getLocation()">
	...
	<!-- Inner Section -->
		<?php  include_once('myprojecttaskfilter-inner.php');?>
    <!-- #END# Inner Section -->
