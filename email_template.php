...
try {
	...
	if (isset($_POST["submit"])) { 
		...
		if (empty($id)) {
			$save = $db->prepare("INSERT INTO tbl_email_templates SET title = :title, content = :content, createdby = :createdby, dateCreated = :today");
			$result  = $save->execute(array(":title" => $title, ":content" => $content, ":createdby"=>$createdby, ":today" => $today));
		} else {
			$save = $db->prepare("UPDATE tbl_email_templates SET title = :title, content = :content, createdby = :createdby, dateCreated = :today WHERE id = :id");
			$result  = $save->execute(array(":title" => $title, ":content" => $content, ":createdby"=>$createdby, ":today" => $today, ":id" => $id));
		}
				...
				<!-- Draggable Handles -->
 				...
 				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
 					<div class="card">
 						<!-- <div class="body"> -->
 						<?php
							include_once('email-template-inner.php');
							?>
 						<!--</div> -->
 					</div>
 				</div>
