...
if ($permission) {
	...
	try {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addindfrm")) {
			...
			$source_data = 0;
			$inddirection = (isset($_POST['inddirection']) && !empty($_POST['inddirection'])) ? $inddirection = $_POST['inddirection'] : 0;;
						...
						if (isset($_POST['inddirectBenfType'])) {
							...
							if (isset($_POST['direct_outcome_dissagragation_type']) && !empty($_POST['direct_outcome_dissagragation_type'])) {
								...
								for ($i = 0; $i < count($disaggregated_type); $i++) {
									...
									if ($result2) {
										if (isset($_POST['direct_outcome_disaggregations']) && !empty($_POST['direct_outcome_disaggregations'])) {
