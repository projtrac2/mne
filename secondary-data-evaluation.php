		...
		if ((isset($_POST["submit"]))) {
			...
			for ($j = 0; $j < count($_POST['location']); $j++) {
				...
				if ($disaggregation) {
					...
					for ($i = 0; $i < count($_POST['answer' . $level3]); $i++) {
						$measurement = $_POST['answer' . $level3][$i];
						$insertSQL = $db->prepare("INSERT INTO tbl_survey_conclusion (formkey, projid, indid, resultstype, resultstypeid, survey_type, level3, disaggregation, measurement, comments, created_by, date_created) VALUES (:formkey, :projid, :indid, :resultstype, :resultstypeid, :surveytype, :level3, :disaggregation, :measurement, :comments, :user, :date)");
						$result = $insertSQL->execute(array(':formkey' => $submissionid, ':projid' => $projid, ':indid' => $indid, ':resultstype' => $resultstype, ':resultstypeid' => $resultstypeid, ':surveytype' => $surveytype, ':level3' => $level3, ':disaggregation' => $disaggregationid[$i], ':measurement' => $measurement, ':comments' => $comments, ':user' => $user, ':date' => $current_date));
					}
				} else {
					$measurement = $_POST['answer' . $level3];
					$insertSQL = $db->prepare("INSERT INTO tbl_survey_conclusion (formkey, projid, indid, resultstype, resultstypeid, survey_type, level3, measurement, comments, created_by, date_created) VALUES (:formkey, :projid, :indid, :resultstype, :resultstypeid, :surveytype, :level3, :measurement, :comments, :user, :date)");
					$result = $insertSQL->execute(array(':formkey' => $submissionid, ':projid' => $projid, ':indid' => $indid, ':resultstype' => $resultstype, ':resultstypeid' => $resultstypeid, ':surveytype' => $surveytype, ':level3' => $level3, ':measurement' => $measurement, ':comments' => $comments, ':user' => $user, ':date' => $current_date));
				}
			}
										...
										<fieldset class="scheduler-border">
											...
											<form method="POST" name="submitevalfrm" action="" enctype="multipart/form-data" autocomplete="off">
												...
												$variableid = $row_variables["id"]; */
														...
														?>
																<div class="col-md-6">
																	<label class="control-label"><?= $disaggregations["disaggregation"] ?> (<?= $unit ?>):</label>
																	<div class="form-line">
																		<input name="answer<?= $locations ?>[]" type="text" class="form-control" placeholder="Enter value" style="border:#CCC thin solid; border-radius: 5px" required />
																	</div>
																</div>
															<?php
															}
