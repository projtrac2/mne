...
$(document).ready(function () {
  ...
  manageItemTable = $("#manageItemTable").DataTable({
    ajax: "ajax/settings/indicator/fetch-selected-indicator-category-items.php",
    order: [],
    'columnDefs': [{
      'targets': [4],
      'orderable': false,
...
function editItem(itemId = null) {
  if (itemId) {
    ...
    $.ajax({
      url: "ajax/settings/indicator/fetch-selected-indicator-category-item.php",
      type: "post",
      data: { itemId: itemId },
      dataType: "json",
      success: function (response) {
          ...
          .bind("submit", function () {
            ...
            if (category && description && editCategoryType) {
              ...
              var formData = new FormData(this);
                // 
              $.ajax({
                url: "ajax/settings/indicator/project-indicator-category-action.php",
                type: form.attr("method"),
                data: formData,
                cache: false,
                contentType: false,
