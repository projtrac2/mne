...
$(document).ready(function() {
  $("#navtitle").addClass("active");  
    ...
    ajax: "ajax/settings/tender/fetch-selected-tender-types-items.php",
    order: [], 
    'columnDefs': [{
      'targets': [4],
      'orderable': false,
  ...
  $("#submitItemForm").on("submit", function(event) {
    ...
    if (name == "") {
      $("#name").after(
        '<p class="text-danger">Tender Type field is required</p>'
      );
      $("#name")
        .closest(".form-input")
        .addClass("has-error");
    ...
    if (description == "") {
      $("#description").after(
        '<p class="text-danger">Type Description field is required</p>'
      );
      $("#description")
        .closest(".form-input")
        .addClass("has-error");
    ...
    if (name && description ) {
      ...
      $.ajax({
        url: "ajax/settings/tender/project-tender-types-action.php",
...
function editItem(itemId = null) {
  if (itemId) {
    ...
    $.ajax({
      url: "ajax/settings/tender/fetch-selected-tender-types-item.php",
      type: "post",
      data: { itemId: itemId },
      dataType: "json",
      success: function(response) {
