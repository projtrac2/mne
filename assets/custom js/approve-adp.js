$(document).ready(function () {
  projectsTable = $("#projectsTable").DataTable({
    ajax: "general-settings/selected-items/fetch-approved-adp",
    order: [],
    columnDefs: [
      {
        targets: [6],
...
function get_op_sum_target(opid, year) {
  let ceiling_year_target = parseInt(
...
function amountfunding(rowno) {
  ...
  if (ceilingval != "" && parseFloat(ceilingval) > 0) {
    if (amountfunding != "" && parseFloat(amountfunding) > 0) {
      ceilingval = parseFloat(ceilingval);
      amountfunding = parseFloat(amountfunding);
      var remaining = ceilingval - amountfunding;

      if (remaining >= 0) {
        $("#source_span_" + rowno).html(
          commaSeparateNumber(parseFloat(remaining))
        );
      } else {
...
function amount_funding(rowno) {
  ...
  if (ceilingval) {
    if (amountfunding) {
      if (parseFloat(amountfunding) > 0) {
        ...
        if (diff >= 0) {
          ...
          $(".source_category_span_row" + source_categoryrow).html(
          ...
          if (remaining < 0) {
            ...
            $(amount_contribute_id).each(function () {
              if ($(this).val()) {
                amount_contribute_val =
                  parseFloat($(this).val()) + amount_contribute_val;
              }
            });
            var diff = source_category_val - amount_contribute_val;
            $(".source_category_span_row" + source_categoryrow).html(
    ...
    $("#source_span_" + rowno).html(commaSeparateNumber(parseInt(ceilingval)));
    $("#amountfunding" + rowno).val("");
    // financierDifference(rowno);
  }
}
  ...
  if (state_ceil != "") {
    if (parseInt(state_ceil) >= 0) {
      if (optotaltarget != "") {
        ...
        if (remainder < 0) {
          alert(
            `Ensure your target adds up to the defined target above of ${optotaltarget}`
          );
          ...
          var state_ceil = $(`#ceilinglocation_target${stateid}${opid}`).val();
          $(`#state_ceil${stateid}${opid}`).html(
            commaSeparateNumber(state_ceil)
          );
          validate_fields(opid, stateid);
        }
      } else {
...
function get_sum(state, id) {
  ...
  if (location_target != "") {
    ...
    if (location_target >= 0) {
      ...
      let ceilinglocation_target = parseInt(
...
function approvePADP(progid = null) {
  if (progid) {
    $.ajax({
      type: "post",
      url: "general-settings/action/adp-edit-action",
      data: { create_padp_div: "create_padp_div", progid: progid },
      dataType: "html",
      success: function (response) {
