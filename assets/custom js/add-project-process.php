...
try {
    ...
    if (isset($_POST['addoutput'])) {
        ...
        if (isset($_POST['projid']) && !empty($_POST['projid'])) {
            $projid = $_POST['projid'];
            $insertSQL1 = $db->prepare("INSERT INTO `tbl_project_details`(unique_key, progid, projid, outputid, indicator, year, duration, budget, total_target) VALUES(:unique_key,:progid, :projid, :output, :indicator, :outputfscyear,  :outputduration, :budget,:outputTarget)");
            $result1  = $insertSQL1->execute(array(":unique_key" => $unique_key, ":progid" => $progid, ":projid" => $projid, ":output" => $output, ":indicator" => $outputIndicator, ":outputfscyear" => $outputfscyear, ":outputduration" => $outputduration, ":budget" => $outputbudget, ":outputTarget" => $outputTarget));
            ...
            $insertSQL1 = $db->prepare("INSERT INTO `tbl_project_details`(unique_key,progid, outputid, indicator, year, duration, budget, total_target) VALUES(:unique_key, :progid, :output, :indicator, :outputfscyear,:outputduration, :budget,:outputTarget)");
            $result1  = $insertSQL1->execute(array(":unique_key" => $unique_key, ":progid" => $progid, ":output" => $output, ":indicator" => $outputIndicator, ":outputfscyear" => $outputfscyear, ":outputduration" => $outputduration, ":budget" => $outputbudget, ":outputTarget" => $outputTarget));
            $last_id = $db->lastInsertId();
    ...
    if (isset($_POST['get_target_div'])) {
        ...
        if ($rows_OutpuData > 0) {
            ...
            for ($i = 0; $i < $years; $i++) {
                ...
                $query_rsprojTarget =  $db->prepare("SELECT SUM(target) as target,  SUM(budget) FROM tbl_project_output_details WHERE indicator ='$indicator' and progid ='$programid' and year ='$projstartyear'");
                $query_rsprojTarget->execute();
                $row_rsprojTarget = $query_rsprojTarget->fetch();
                ...
                // targets
                $containerTH .= '<th>' . $projstartyear  . '/' . $endyear . '
                ...
                $containerTB .= '<td> 
                <input type="number" data-id=""  name="target_year' . $outputIds . '[]" placeholder="target"  id="target_year' . $outputIds . $projstartyear . '" class="form-control workplanTarget' . $outputIds . '" min="0" onkeyup=get_op_sum_target(' . $outputIds . ',' . $projstartyear . ') required >
                </td>';

                // budgets
                $containerTH .= '<th>' . $projstartyear  . '/' . $endyear . '
                <input type="hidden" class="b_output_years' . $outputIds  . '" name="output_years' . $outputIds  . '[]" value="' . $projstartyear . ' " >
                ...
                $containerTB .= '<td> 
                <input type="number" data-id=""  name="budget_year' . $outputIds . '[]" placeholder="Budget"  id="budget_year' . $outputIds . $projstartyear . '" class="form-control output_budget' . $outputIds . '" min="0" onkeyup=get_op_sum_budget(' . $outputIds . ',' . $projstartyear . ') required >
                </td>';
