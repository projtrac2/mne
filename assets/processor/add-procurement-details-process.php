...
try {
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add_budget_line_frm")) {
				...
				for ($j = 0; $j < count($task); $j++) {
					...
					if (!empty($dunitcost)  && !empty($dunit)  && !empty($dtotalunits)) {
						...
						for ($pt = 0; $pt < count($dunitcost); $pt++) {
							...
							$result  = $insertSQL->execute(array(":projid" => $projid, ":outputid" => $outputid, ':costlineid' => $costlineid[$pt], ':tasks' => $taskid, ':description' => $description[$pt], ':unit' => $dunit[$pt], ':unit_cost' => $dunitcost[$pt], ':units_no' => $dtotalunits[$pt], ":created_by" => $user_name, ":date_created" => $datecreated));
						}
					}
				}
			}
		...
		if ($payment_plan == 1) {
			if (isset($_POST['milestone_ids'])) {
				$counter = count($_POST['milestone_ids']);
				for ($i = 0; $i < $counter; $i++) {
					$milestone = $_POST['milestone_ids'][$i];
					$value = 0;
					$insertSQL = $db->prepare("INSERT INTO `tbl_project_payment_plan_details` (projid, milestone_id, value, created_by, created_at) VALUES(:projid, :milestone_id, :value, :created_by, :created_at)");
					$insertSQL->execute(array(":projid" => $projid, ":milestone_id" => $milestone, ":value" => $value, ":created_by" => $user_name, ":created_at" => $datecreated));
				}
			}

			$query_rsPayement_plan =  $db->prepare("SELECT * FROM  tbl_project_payment_plan_details WHERE projid=:projid ORDER BY id ASC");
			$query_rsPayement_plan->execute(array(":projid" => $projid));
			$rows_rsPayement_plan = $query_rsPayement_plan->fetch();
			$total_rsPayement_plan = $query_rsPayement_plan->rowCount();

			if ($total_rsPayement_plan > 0) {
				$plan_ids = [];
				do {
					$msid = $rows_rsPayement_plan['milestone_id'];
					$id = $rows_rsPayement_plan['id'];
					$last_plan = end($plan_ids);
					$query_milestones = $db->prepare("SELECT * FROM tbl_milestone WHERE msid <=:msid AND msid > :start_msid AND projid=:projid");
					$query_milestones->execute(array(":msid" => $msid, ":start_msid" => $last_plan, ":projid" => $projid));
					$rows_milestones = $query_milestones->fetch();
					$total_milestones = $query_milestones->rowCount();

					if ($total_milestones > 0) {
						do {
							$msid = $rows_milestones['msid'];
							$insertSQL = $db->prepare("UPDATE tbl_milestone SET payment_plan_id=:payment_plan_id WHERE projid=:projid AND msid=:msid");
							$results  = $insertSQL->execute(array(':payment_plan_id' => $id, ":projid" => $projid, ":msid" => $msid));
						} while ($rows_milestones = $query_milestones->fetch());
					}
					$plan_ids[] = $msid;
				} while ($rows_rsPayement_plan = $query_rsPayement_plan->fetch());
			}
		} else if ($payment_plan == 3) {
			if (isset($_POST['milestone_name'])) {
				$counter = count($_POST['milestone_name']);
				for ($i = 0; $i < $counter; $i++) {
					$milestone = $_POST['milestone_name'][$i];
					$value = $_POST['percentage'][$i];
					$insertSQL = $db->prepare("INSERT INTO `tbl_project_payment_plan_details` (projid, milestone_id, value, created_by, created_at) VALUES(:projid, :milestone_id, :value, :created_by, :created_at)");
					$insertSQL->execute(array(":projid" => $projid, ":milestone_id" => $milestone, ":value" => $value, ":created_by" => $user_name, ":created_at" => $datecreated));
				}
			}
		...
		$insertSQL = $db->prepare("UPDATE tbl_projects SET projstage=:projstage, payment_plan=:payment_plan WHERE projid=:projid");
		$results  = $insertSQL->execute(array(":projstage" => $projstage, ":payment_plan" => $payment_plan, ":projid" => $projid));
		if ($results) {
	...
	if (isset($_POST['getprocurementdetails'])) {
		...
		if ($totalRows_rsProjFinancier > 0) {
			do {
				...
				$query_rsFunder->execute(array(":financier" => $financierid));
