...
try{
        ...
        if($formid !=""){ 
            ...
            if($totalrows_rs_projects > 0 ){		 
				foreach($rows_projects as $rows_rs_projects){
					...
					$detailslink = '<a href="'.$url.'public-survey-form?prj='.$encode_prjid.'&fm='.$encode_frmid.'&em='.$encode_emailid.'" class="btn bg-light-blue waves-effect" style="margin-top:10px; margin-left:-9px">CLICK HERE TO OPEN FORM</a>';
					
					$mainmessage = ' Dear ' . $fullname . ',
					<p>Please note you have been assigned to do project '.$formname.' survey as per the details below:</p>
					...
					$subject = "Project ".$formname." Survey";
    ...
    if(isset($_POST['insert'])){
        ...
        for($i=0; $i<count($level3); $i++){ 
            ...
            for($j=0; $j<count($disaggregation); $j++){ 
                $query_rsInsert = $db->prepare("INSERT INTO tbl_indicator_level3_disaggregations(projid, form_id, indicatorid, level3, disaggregations)  VALUES (:projid,:form_id,:indicatorid, :level3, :disaggregations)");
                $results =$query_rsInsert->execute(array(":projid"=>$projid,":form_id"=>$form_id,":indicatorid"=>$indicatorid,":level3"=>$level3[$i], ":disaggregations"=>$disaggregation[$j]));
            }
        }
        ...
        if($results){
			...
			for($i=0; $i<count($level3); $i++){
				...
				$state = $row_rsComm['state'];
					...
					$query_rs_level3 = $db->prepare("SELECT * FROM tbl_indicator_level3_disaggregations WHERE indicatorid = :indicatorid AND  projid=:projid AND form_id=:form_id AND level3=:level3");
					$query_rs_level3->execute(array(":indicatorid"=>$indicatorid, ":projid"=>$projid,":form_id"=>$form_id, ":level3"=>$level3id));
					$row_rs_level3 = $query_rs_level3->fetch();
					$totalRows_rs_level3 = $query_rs_level3->rowCount();
				if($totalRows_rs_level3 > 0){
					$counter =0; 
					do{ 
						$counter++;
						$disaggregations = $row_rs_level3['disaggregations']; 
						$id = $row_rs_level3['id']; 
						$html .=
						'<tr>
							<td width="10%">
							...
							<td width="30%">
								'. $disaggregations .'
								<input name="disaggregation'. $level3id .'[]" type="hidden" id="disaggregation" value="'. $id .'" />
							</td>
							<td width="30%"> 
								<div class="form-input">';
									if($respondents_type ==1){
										$html .='
											<select name="enumerators'. $level3id .'[]" class="form-control show-tick" id="enumerators" style="border:#CCC thin solid; border-radius:5px; width:98%" data-live-search="true" required>
											...
											$html .='<input type="email" name="enumerators'. $level3id .'[]" id="mail" placeholder="example@mail.com" class="form-control" style="border:#CCC thin solid; border-radius: 5px" required>';                                                                      
										}
									$html .= 
								'</div>
							</td>  
						</tr>';   
					}while($row_rs_level3 = $query_rs_level3->fetch());
				}
			}
        } 
    ...
    if ((isset($_POST["MM_insert"])) ) {
        ...
        if(isset($_POST['directben']) && !empty($_POST['directben'])){
            ...
            for($i=0; $i<count($state); $i++){
                $level3 = $state[$i];
                $disaggregations =$_POST['disaggregation'.$level3];
                $enumerators =$_POST['enumerators'.$level3];
                for($j=0; $j<count($disaggregations); $j++){
                    $diss = $disaggregations[$j];
                    $enumerators = $enumerators[$j];  
                    $insertBeneficiary = $db->prepare("INSERT INTO tbl_indicator_baseline_survey_details(formid, level3,location_disaggregation, enumerators, added_by,date_added) VALUES(:formid, :level3,:diss,:enumerators, :created_by, :date_created)");
                    $result2  = $insertBeneficiary->execute(array(":formid"=>$formid, ":level3"=>$level3,":diss"=>$diss,":enumerators"=>$enumerators,":created_by"=>$created_by, ":date_created"=>$date_created));
                }
            }  
        ...
        if(isset($_POST['indirectben']) && !empty($_POST['indirectben'])){
            ...
            for($j=0; $j<count($_POST['enumerators']); $j++){
                ...
                $level3 = $_POST['level3'][$j];
                $insertBeneficiary = $db->prepare("INSERT INTO tbl_indicator_baseline_survey_details(formid, level3, enumerators, added_by, date_added) VALUES(:formid, :level3, :enumerators, :created_by, :date_created)");
                $result2  = $insertBeneficiary->execute(array(":formid"=>$formid, ":level3"=>$level3, ":enumerators"=>$enumerators, ":created_by"=>$created_by, ":date_created"=>$date_created));
            }
    ...
    if(isset($_GET['more'])){
        $formid = $_GET['formid'];
        $query_baseline = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms f 
		
		INNER JOIN tbl_indicator_baseline_survey_details d on d.formid=f.id
       
        INNER JOIN tbl_project_evaluation_submission s ON s.formid=f.id
		 
        WHERE f.id=:formid GROUP BY s.email ORDER BY f.id ASC"); 
                    ...
                    if ($count_baseline > 0) {
                        ...
                        while($rows_baseline = $query_baseline->fetch()) {
                            ...
                            if($count_projects > 0){ 
                                ...
                                $query_count = $db->prepare("SELECT * FROM tbl_project_evaluation_submission WHERE formid='$formid' AND email = '$email'");
                                $query_count->execute();
                                ...
                                <tr class="bg-orange">
                                    ...
                                    <td style="width:20%">' . $count_count . '</td>  
                                </tr>';
                            }
                        }
                    } else {
