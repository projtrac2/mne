...
function validate_project_duration() {
  ...
  if ($("#projduration1").val() != "") {
    ...
    if (endyearDate <= progendyearDate) {
      var Edate = "";
      if (projduration > 365) {
        Edate = projendyear + "/" + (parseFloat(projendyear) + 1);
      } else {
        var end = (parseFloat(projendyear));
        var proj = (projendyear == starting_year) ? projendyear + 1 : projendyear;
        Edate = starting_year + "/" + proj;
      }
...
function get_sum_op_budget_year(opid, year) {
  ...
  if (remainder < 0) {
    alert(
      `The sum total of year values cannot exceed  ${opid_name} ceiling(${ceiling_year_budget})`
    );
    $(`#budget_year${opid}${year}`).val("");
    let total_sum = 0;
    $(`.output_budget${opid}`).each(function () {
