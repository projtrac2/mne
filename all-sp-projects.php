...
try {
	if ($rows_count > 0) {
		...
		do {
			...
			$projname =  $row_rsUpP['projname'];
			$progid =  $row_rsUpP['progid'];
			$projcode =  $row_rsUpP['projcode'];
			$projcost =  $row_rsUpP['projcost'];
			$projstage =  $row_rsUpP['projstage'];
			$projstatus =  $row_rsUpP['projstatus'];
			$location = explode(",", $row_rsUpP['projlga']);
			$fscyear = $row_rsUpP['projfscyear'];
			$row_progid = $row_rsUpP['progid'];
			$project_start_date =  $row_rsUpP['projstartdate'];
			$project_end_date =  $row_rsUpP['projenddate'];
			$projcategory =  $row_rsUpP['projcategory'];
			$percent2 =  $row_rsUpP['progress'];
...
?>
			<tr id="rows">
				...
				<td>
					<a href="view-project-maps.php?projid=<?php echo $row_rsUpP['projid']; ?>" id="" class="" style="color:indigo"><?= implode(",", $locations); ?></a>
				</td>
