...
try{
	...
	$query_rsStPlan =  $db->prepare("SELECT * FROM tbl_strategicplan WHERE id='$stplan'");
	$query_rsStPlan->execute();		
	$row_rsStPlan = $query_rsStPlan->fetch();
	$totalRows_rsStPlan = $query_rsStPlan->rowCount();
	
	$strategicplan = $row_rsStPlan["plan"];
	$vision = $row_rsStPlan["vision"];
	$mission = $row_rsStPlan["mission"];
	$datecreated = $row_rsStPlan["date_created"];

	$query_KRA =  $db->prepare("SELECT * FROM tbl_key_results_area WHERE spid='$stplan' ORDER BY kra ASC");
	$query_KRA->execute();		
...
<body class="theme-blue">
	...
	<!-- Inner Section -->
		<?php  include_once('strategic-plan-framework-inner.php');?>
    <!-- #END# Inner Section -->
