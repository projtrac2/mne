<form method="POST" name="submitevalfrm" id="survey" action="" enctype="multipart/form-data" autocomplete="off">
	...
	if(isset($_GET['fm']) && !empty($_GET['fm'])){
		...
		$query_questions = $db->prepare("SELECT * FROM tbl_project_evaluation_questions WHERE resultstype =:resultstype AND projid=:projid");
		$query_questions->execute(array(":resultstype" => $resultstype, ":projid" => $projid));
									...
									<tr><input type="hidden" name="questionid[]" id="questionid" value="'.$questionid.'">
										...
										<td>
											...
											<div class="form-group form-line">';
												if($answertype == 1){
													echo '<input name="answer'.$questionid.'" type="number" id="answer' . $questionid . '" style="border:#CCC thin solid; border-radius:5px; width:95%" required placeholder="answer'.$questionid.'">';
												}
												elseif($answertype == 2){
												...
												elseif($answertype == 4){
													$answerlabels = explode(",",$answerlabels);
													$dropdown = '<select name="answer'.$questionid.'" id="answer' . $questionid . '" style="border:#CCC thin solid; border-radius:5px; width:95%" data-live-search="false" required>
								...
								foreach ($row_questions as $field){
									...
									<tr><input type="hidden" name="questionid[]" id="questionid" value="'.$questionid.'">
										...
										<td>
											...
											<div class="form-group form-line">';
												if($answertype == 1){
													echo '<input name="answer'.$questionid.'" type="number" id="answer' . $questionid . '" style="border:#CCC thin solid; border-radius:5px; width:95%" required placeholder="Enter number">';
												}
												elseif($answertype == 2){
													$answerlabels = explode(",",$answerlabels);
													$multiples = '';
