...
include_once 'projtrac-dashboard/resource/utilities.php';
	...
	if(isset($_POST['risklevel']) && !empty($_POST['risklevel'])){
		$rskid = $_POST['issueid'];
		$projid = $_POST['projid'];
		$score = $_POST['risklevel'];
		$mitigation = $_POST['mitigation'];
		$notes = $_POST['notes'];
		$date = $_POST['dateanalysed'];
		$user = $_POST['username'];
		$status = 3;
		...
		$query_timeline->execute(array(":stage" => $status));		
		...
		//--------------------------------------------------------------------------							  
		$assignrisk = $db->prepare("INSERT INTO tbl_project_riskscore (projid,issueid,score,mitigation,notes,date_analysed,added_by) VALUES (:projid, :issueid, :score, :mitigation, :notes, :date, :user)");
		$assignrisk->execute(array(':projid' => $projid, ':issueid' => $rskid, ':score' => $score, ':mitigation' => $mitigation, ':notes' => $notes, ':date' => $date, ':user' => $user));
		
		$updateQuery = $db->prepare("UPDATE tbl_projissues SET status=:status WHERE id=:rskid");
		$update = $updateQuery->execute(array(':status' => $status, ':rskid' => $rskid));
								
		$query_teamleader =  $db->prepare("SELECT fullname, title, email FROM tbl_projteam2 t inner join tbl_projmembers m on m.ptid=t.ptid WHERE projid = :projid and m.role=1");
		$query_teamleader->execute(array(":projid" => $projid));		
		$row_teamleader = $query_teamleader->fetch();
		$teamleader = $row_teamleader["title"].".".$row_teamleader["fullname"];
		...
		$issuedesc = $issuedetails["observation"];
		$projcode = $issuedetails["projcode"];
		...
		$issuelink = '<a href="'.$url.'projectissueslist?proj='.$projid.'&user='.$teamleaderid.'" target="_blank">Check the details</a>';
		...
		$mail->addAddress($teamleaderemail, $teamleader);     // Add a recipient              // Name is optional
