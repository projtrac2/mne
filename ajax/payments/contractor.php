...
try {
    ...
    function get_milestone_based_details($projid, $payment_plan_id)
    {
        ...
        return array("success" => true, "milestones" => $milestones, "request_percentage" => number_format($percentage, 2), "request_amount" => $request_amount);
    }
    ...
    if (isset($_GET['get_more_info'])) {
        ...
        if ($total_rsPayement_reuests > 0) {
            ...
            $query_rsPayement_reuests_comments =  $db->prepare("SELECT * FROM  tbl_contractor_payment_request_comments WHERE request_id=:request_id");
    ...
    if (isset($_POST['disburse_contractor_payment'])) {
        ...
        if ($msg) {
            $sql = $db->prepare("INSERT INTO tbl_payments_disbursed (request_id,payment_mode, comments, receipt,paid_to, date_paid,request_type,created_by,created_at) VALUES(:request_id,:payment_mode,:comments,:receipt,:paid_to,:request_type,:date_paid,:created_by,:created_at) ");
            $result = $sql->execute(array(":request_id" => $request_id, ":payment_mode" => $payment_mode, ":comments" => $comments, ":receipt" => $receipt, ":paid_to" => $paid_to, ":date_paid" => $date_paid, ":request_type" => 2, ":created_by" => $created_by, ":created_at" => $created_at));
