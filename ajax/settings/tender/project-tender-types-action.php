...
try {
	...
	if (isset($_POST["newitem"])) {
		...
		$status = 1;
		$sql = $db->prepare("INSERT INTO tbl_tender_type (type, description,status) VALUES(:name, :description,:status)");
	...
	if (isset($_POST["edititem"])) {
		...
		$updateQuery = $db->prepare("UPDATE tbl_tender_type SET type=:name, description=:description WHERE id=:itemid");
	...
	if (isset($_POST["deleteItem"])) {
		...
		$stmt = $db->prepare("SELECT * FROM `tbl_tender_type` where id=:itemid");
		$stmt->execute([':itemid' => $itemid]);
		$stmt_results = $stmt->fetch();

		$stmt_results['status'] == 1 ? $updateStatus = '0' : $updateStatus = '1';

		$deleteQuery = $db->prepare("UPDATE `tbl_tender_type` SET status=:status WHERE id=:itemid");
