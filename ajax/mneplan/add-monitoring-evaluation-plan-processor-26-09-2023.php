...
if (isset($_POST['addimpact'])) {
    if (isset($_POST['impact']) && !empty($_POST['impact'])) {
        ...
        if ($result1) {
            $query_timeline = $db->prepare("SELECT * FROM tbl_project_workflow_stage_timelines WHERE category = 'Baseline' AND stage = 6");
            $query_timeline->execute();
            $row_timeline = $query_timeline->fetch();
            $time = $row_timeline ? $row_timeline["time"] : 0;
            $unit = $row_timeline ? $row_timeline["units"] : 0;
