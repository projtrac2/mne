...
try {
    if (isset($_GET['get_issue_more_info'])) {
		...
		$query_issue_details = $db->prepare("SELECT i.projid, i.issue_area, i.issue_priority, issue_description, m.description AS impact, category, recommendation, status, i.created_by AS monitor, i.date_created AS issuedate FROM tbl_projissues i INNER JOIN tbl_projrisk_categories c ON c.catid=i.risk_category inner join tbl_risk_impact m on m.id=i.issue_impact WHERE i.id=:issueid");
		...
		if($issue_areaid == 2){
			$issue_more_info .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<fieldset class="scheduler-border">
					<legend  class="scheduler-border" style="background-color:#c7e1e8; border-radius:3px;"><i class="fa fa-list" aria-hidden="true"></i> Issue Area Details (Scope)</legend>
					<div class="table-responsive">
						<table class="table table-bordered table-striped table-hover">
							...
							while ($row_adjustments = $query_adjustments->fetch()){
								...
								$issue_more_info .= '<tr class="adjustments '.$issueid.'" style="background-color:#e5e5e5">
									<td>' . $scopecount.'</td>
									<td>' . $subtask . '</td>
									<td>' . $units . ' </td>
									<td>' . $timeline . '</td>
	...
	if(isset($_GET['issue_status']) && !empty($_GET['issue_status'])) 
	{
		...
		}elseif($statusid=="RestoreAndAdjust"){
			$projchangedstatus = $projstatus;
			$projstatusselect =	'
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<label><font color="#174082">Project Area to Adjust:</font></label>
