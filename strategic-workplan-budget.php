...
require 'authentication.php';
	...
	if(isset($_GET["plan"]) && !empty($_GET["plan"])){
		...
		if(isset($_GET["finyear"]) && !empty($_GET["finyear"])){
			...
			$query_annualplanyear =  $db->prepare("SELECT * FROM tbl_fiscal_year WHERE id='$finyear'");
			$query_annualplanyear->execute();
			$row_annualplanyear = $query_annualplanyear->fetch();
			$annualplanyear = $row_annualplanyear["year"];
			$annualplanyearid = $row_annualplanyear["id"];
		
			$query_planobjectives = $db->prepare("SELECT o.id, o.objective, o.outcome, i.indicator_name FROM tbl_projects p inner join tbl_programs g on g.progid=p.progid inner join tbl_strategic_plan_objectives o on o.id=g.progobjective inner join tbl_key_results_area k on k.id=o.kraid inner join tbl_strategicplan sp on sp.id=k.spid inner join tbl_indicator i on i.indid=o.kpi WHERE sp.id='$stplan' and p.projfscyear='$annualplanyearid' GROUP BY o.id");
			$query_planobjectives->execute();
			$totalRows_planobjectives = $query_planobjectives->rowCount();
		}else{
			$query_planobjectives = $db->prepare("SELECT o.id, o.objective, o.outcome, i.indicator_name FROM tbl_strategic_plan_objectives o inner join tbl_key_results_area k on k.id=o.kraid inner join tbl_strategicplan sp on sp.id=k.spid inner join tbl_indicator i on i.indid=o.kpi WHERE sp.id='$stplan'");
			$query_planobjectives->execute();
			$totalRows_planobjectives = $query_planobjectives->rowCount();
		}
	}
...
<body class="theme-blue">
	...
	<section class="content" style="margin-top:-20px; padding-bottom:0px">
		<?php 
			include_once('strategic-workplan-budget-inner.php');
		?>
	</section>
