    ...
    if(array_key_exists($key, $data)){
        ...
        if($_POST[$key] != null){
            ...
            if(filter_var($_POST[$key], FILTER_VALIDATE_EMAIL) === false){
                $form_errors[] = $data . " is not a valid email address";
            }
        }
    }
    return $form_errors;
...
function checkDuplicateEntries($table, $column_name, $value, $db){
    try{
        $sqlQuery = "SELECT * FROM $table WHERE $column_name=:$column_value";
        $statement = $db->prepare($sqlQuery);  
        $statement->execute(array(":$column_value" => $value));
...
function isCookieValid($db){
    ...
    if (isset($_COOKIE['rememberUserCookie'])) {
         ...
         * */
        $sqlQuery = "SELECT * FROM afs_sponsor_login WHERE id = :id";
        $statement = $db->prepare($sqlQuery);
...
function prepLogin ($id, $email, $name, $remember){
    $_SESSION['id'] = $id;
