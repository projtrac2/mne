...
try{
    ...
    $query_pendinginspections = $db->prepare("SELECT * FROM tbl_inspection_assignment WHERE status=0 OR status = 1");
	$query_pendinginspections->execute();
	$count_pendinginspections = $query_pendinginspections->rowCount();

    // complete comple
	$query_inspectionsdone = $db->prepare("SELECT * FROM tbl_inspection_assignment WHERE status=1 OR status = 2");
	$query_inspectionsdone->execute();
	$count_inspectionsdone = $query_inspectionsdone->rowCount();
}
            ...
            <div class="row clearfix">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <!-- <div class="body"> -->
							<?php
							include_once('project-inspections-list-inner.php');
							?>
						<!--</div> -->
                    </div>
                </div>
