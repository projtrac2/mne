...
try{	
	...
	$query_op = $db->prepare("SELECT * FROM tbl_indicator WHERE active = '1'");
	$query_op->execute();
	$totalRows_op = $query_op->rowCount();
	

	if($_GET["btn_search"] && !empty($_GET["indicator"])){
		$opindid = $_GET["indicator"];
		$query_ind = $db->prepare("SELECT i.indicator_name, u.unit, i.indid FROM tbl_progdetails d inner join tbl_programs g ON g.progid = d.progid inner join tbl_strategic_plan_objectives o on o.id=g.strategic_obj inner join tbl_key_results_area k ON k.id=o.kraid inner join tbl_indicator i ON i.indid = d.indicator inner join tbl_measurement_units u on u.id=i.indicator_unit WHERE k.spid = '$stplan' and i.indid= '$opindid' GROUP BY i.indid");
		$query_ind->execute();
		$totalRows_ind = $query_ind->rowCount();		
	}else{
		$query_ind = $db->prepare("SELECT i.indicator_name, u.unit, i.indid FROM tbl_progdetails d inner join tbl_programs g ON g.progid = d.progid inner join tbl_strategic_plan_objectives o on o.id=g.strategic_obj inner join tbl_key_results_area k ON k.id=o.kraid inner join tbl_indicator i ON i.indid = d.indicator inner join tbl_measurement_units u on u.id=i.indicator_unit WHERE k.spid = '$stplan' GROUP BY i.indid");
		$query_ind->execute();
		$totalRows_ind = $query_ind->rowCount();
	}
...
<body class="theme-blue">
	...
	<section class="content" style="margin-top:-20px; padding-bottom:0px">
		<?php 
		include_once('output-indicators-tracking-table-inner.php');
		?>
	</section>
