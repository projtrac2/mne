...
if ($permission) {
  ...
  try {
    ...
    $count_impact_baseline_survey = $query_impact_baseline_survey->rowCount();

    $newsurveys = 0;
	$impactid = "";

    if ($count_impact_baseline_survey > 0) {
      while ($row = $query_impact_baseline_survey->fetch()) {
        $impactid = $row["id"];
    ...
    $query_active_survey = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=2 and created_by=:responsible ORDER BY id ASC");
    $query_active_survey->execute(array(":responsible" => $user_name));
    if ( $designation == 1) {
      $query_active_survey = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=2 ORDER BY id ASC");
    ...
    $query_survey_data = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 and created_by=:responsible ORDER BY id ASC");
    $query_survey_data->execute(array(":responsible" => $user_name));
    if ($designation == 1) {
		$query_survey_data = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 ORDER BY id ASC");
	...
	while($row_survey_data = $query_survey_data->fetch()){
		...
		$query_impact_evaluation_concluded = $db->prepare("SELECT * FROM tbl_survey_conclusion WHERE resultstype=1 AND resultstypeid=:resultstypeid AND projid=:projid");
		$query_impact_evaluation_concluded->execute(array(":resultstypeid" => $impactid, ":projid" => $projid));
    ...
    $query_survey_submissions = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 and created_by=:responsible ORDER BY id ASC");
    $query_survey_submissions->execute(array(":responsible" => $user_name));
    if ( $designation == 1) {
      $query_survey_submissions = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 ORDER BY id ASC");
    ...
    while ($rows_survey_submissions = $query_survey_submissions->fetch()) {
      ...
      $query_evaluation_conclusion = $db->prepare("SELECT * FROM tbl_survey_conclusion WHERE resultstype = 1 AND resultstypeid=:resultstypeid");
      $query_evaluation_conclusion->execute(array(":resultstypeid" => $impactid));
    ...
    $query_survey_conclusion = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 and created_by=:responsible ORDER BY id ASC");
    $query_survey_conclusion->execute(array(":responsible" => $user_name));
    if ( $designation == 1) {
      $query_survey_conclusion = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 ORDER BY id ASC");
        ...
        <div class="block-header">
          ...
          <div class="header" style="padding-bottom:0px; margin-left:10px; margin-right:10px">
            <div class="button-demo" style="margin-top:-15px">
              ...
              <a href="impact-evaluation-secondary-data-source.php" class="btn bg-light-blue waves-effect" style="margin-top:10px; margin-left:-9px">Records Data Source</a>
            </div>
          </div>
        </div>
                      ...
                      <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        ...
                        <tbody>
                          ...
                          if ($count_active_survey > 0) {
                            ...
                            echo '<td colspan="7">No active survey</td>';
                          }
                          ?>
                        </tbody>
                      </table>
                  ...
                  <div id="menu2" class="tab-pane fade">
                    ...
                    <div class="body">
                      <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        ...
                        <tbody>
                          <?php
							$query_survey_data_inner = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 and created_by=:responsible ORDER BY id ASC");
							$query_survey_data_inner->execute(array(":responsible" => $user_name));
							if ($designation == 1) {
								$query_survey_data_inner = $db->prepare("SELECT * FROM tbl_indicator_baseline_survey_forms WHERE resultstype = 1 AND status=3 ORDER BY id ASC");
							...
							if ($count_survey_data_inner > 0) {
								...
								echo '<td colspan="7">No concluded survey</td>';
							}
							?>
                        </tbody>
                      </table>
                            ...
                            while ($rows_survey_conclusion = $query_survey_conclusion->fetch()) {
                              ...
                              if ($count_projects > 0) {
                                ...
                                if ($totalsamplesize == $count_count) {
                                  ...
                                  $query_outcome_evaluated = $db->prepare("SELECT * FROM tbl_survey_conclusion WHERE resultstype=1 AND resultstypeid=:resultstypeid");
                                  $query_outcome_evaluated->execute(array(":resultstypeid" => $impactid));
                                  ...
                                  if ($rows_outcome_evaluated == 0) {
                                    ...
                                    echo '
										<tr>
										  ...
										  <td style="width:10%">' . $enddate . '</td>
										  <td style="width:10%">
											<a type="button" class="badge bg-purple" href="survey-conclusion.php?results=' . $impactidencoded . '&resultstype=1">
											  Conclude
											</a>
										  </td>
										</tr>';
