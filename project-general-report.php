...
try {
    ...
    $query_rsgraphs = $db->prepare("SELECT ((SUM(actualoutput) /d.total_target)*100) as Rate, g.output FROM tbl_monitoringoutput m 
    INNER JOIN tbl_project_details d ON d.id = m.opid 
    INNER JOIN tbl_progdetails g ON g.id = d.outputid WHERE m.projid='$projid' GROUP BY opid  ");
    $query_rsgraphs->execute();
    // $row_rsgraphs = $query_rsgraphs->fetch();
    $totalRows_rsgraphs = $query_rsgraphs->rowCount();
 
    $chart_data = '';
    while($row_rsgraphs = $query_rsgraphs->fetch()){ 
        $chart_data .='{ label: "'.$row_rsgraphs["output"].'",  y: '.$row_rsgraphs["Rate"].' },';
    } 
...
<body class="theme-blue">
    ...
    <section class="content" style="margin-top:-20px; padding-bottom:0px">
        <div class="container-fluid">
            <?php
            include_once('project-general-report-inner.php');
            ?>
        </div>
    </section>
    ...
    <script src="projtrac-dashboard/js/pages/charts/morrischarts.js"></script>
